...
<AccordionGroup>
  <Accordion title="Supabase 🔒" icon="database">
    To connect to Supabase, use the following credentials:

    - **URL:** The unique URL of your Supabase instance.
    - **Anon Key:** The public API key for accessing Supabase client-side.
    - **Service Key:** The secret API key for accessing Supabase server-side (use with caution).
