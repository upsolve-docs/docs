...
Obtain the squared elements of an array column with [transform()](/sql/functions/array#transform):
...
The function [transform()](/sql/functions/array#transform) can be also employed to safely cast the elements
...
with [transform()](/sql/functions/array#transform):
...
with [any_match()](/sql/functions/array#any-match):
...
Capitalize the first word in a string via [regexp_replace()](/sql/functions/regexp#regexp-replace):
...
by making use of [reduce_agg()](/sql/functions/aggregate#reduce-agg):
