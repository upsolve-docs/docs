...
These functions and operators operate on [date and time data types](/sql/reference/types#date-time-data-types).
...
Returns the current [timestamp with time zone](/sql/reference/types#timestamp-with-time-zone-data-type) as of the start of the query, with
...
localtimestamp(p)
```
Returns the current [timestamp](/sql/reference/types#timestamp) as of the start
...
```
This is an alias for [week](#week).
...
yow(x) -> bigint
```
This is an alias for [year_of_week](#year_of_week).
