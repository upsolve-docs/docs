...
[Pattern Comparison: LIKE](/sql/functions/comparison#pattern-comparison-like)
...
Invalid UTF-8 data can be corrected with [from_utf8()](#from-utf8).
...
The [lower()](#lower) and [upper()](#upper) functions do not perform
...
```
See [format()](/sql/functions/conversion#format).
...
`IN` keyword for the arguments. See also [strpos()](#strpos).
</Note>
...
```
This is an alias for [substring()](#substring).
