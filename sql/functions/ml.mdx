...
numerical values, [features()](#features):
...
The output from [features()](#features) can be directly passed to ML functions.
...
[classify()](#classify) returns the predicted label by using the trained model.
...
You can use [learn_libsvm_classifier()](#learn-libsvm-classifier) to control the internal parameters of the model.
...
Internally, the model is trained by [libsvm](https://www.csie.ntu.edu.tw/~cjlin/libsvm/).
[learn_libsvm_regressor()](#learn-libsvm-regressor) provides you a way to control the training process.
