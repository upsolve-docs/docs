...
[json_exists](#json-exists), [json_query](#json-query), and [json_value](#json-value). Each of them
...
[json-array](#json-array), and [json_object](#json-object).
...
listing. Path modes are explained in [JSON path modes](#json-path-modes).
</Note>
...
- null literal
  ...
  It has the semantics of the JSON null, not of SQL null. See [comparison rules](#json-comparison-rules).
...
| --------- | ----------- | -------- |
| Performing an operation which requires a non-array on an array, e.g.:<br />`$.key` requires a JSON object<br />`$.floor()` requires a numeric value | ERROR | The array is automatically unnested, and the operation is performed on each array element. |
| Performing an operation which requires an array on an non-array, e.g.:<br />`$[0]`, `$[*]`, `$.size()` | ERROR | The non-array item is automatically wrapped in a singleton array, and the operation is performed on the array. |
| A structural error: accessing a non-existent element of an array or a non-existent member of a JSON object, e.g.:<br />`$[-1]` (array index out of bounds)<br />`$.key`, where the input JSON object does not have a member `key` | ERROR | The error is suppressed, and the operation results in an empty sequence. |
...
[JSON path syntax and semantics](#json-path-syntax-and-semantics).
...
<Note>
Cast operations with supported [character string types](/sql/reference/types#string) treat the input as a string, not validated as JSON.
...
The [json_query function](#json-query) provides a more powerful and
...
```
Like [json_extract](#json-extract), but returns the result value as a string (as opposed
...
<Note>
  [json_format function](#json-format) and `CAST(json AS VARCHAR)` have completely
  different semantics.

  [json_format function](#json-format) serializes the input JSON value to JSON text conforming to
...
This is inverse function to [json_format function](#json-format):
...
```
Like [json_extract](#json-extract), but returns the size of the value.
