...
The window can be specified in two ways (see [WINDOW clause](/sql/reference/select#window-clause)):
...
All [aggregate functions](/sql/functions/aggregate) can be used as window functions by adding the `OVER`
...
aggregation](/sql/functions/aggregate#ordering-during-aggregation) is not supported.
...
[rank()](#rank), except that tie values do not produce gaps in the sequence.
...
is `(r - 1) / (n - 1)` where `r` is the [rank()](#rank) of the row and
