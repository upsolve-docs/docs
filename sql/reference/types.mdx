...
Upsolve has a set of built-in data types, described below.

## Upsolve type support and mapping

Connectors to data sources are not required to support all Upsolve data types
described on this page. If there are data types similar to Upsolve's that are used
on the data source, the connector may map the Upsolve and remote data types to
each other as needed.

Depending on the connector and the data source, type mapping may apply
in either direction as follows:

- **Data source to Upsolve** mapping applies to any operation where columns in the
  data source are read by Upsolve, such as a [SELECT](/sql/reference/select) statement, and the
  underlying source data type needs to be represented by a Upsolve data type.
...
<Note>Binary strings with length are not yet supported: `varbinary(n)`</Note>
...
See also [Date and Time functions](/sql/functions/datetime)
...
operators](/sql/functions/datetime) can return timestamp values.
...
[HyperLogLog](https://wikipedia.org/wiki/HyperLogLog) data sketch. See `/functions/hyperloglog`.

### `HyperLogLog`

A HyperLogLog sketch allows efficient computation of [approx_distinct()](/sql/functions/aggregate#approx-distinct). It starts as a
sparse representation, switching to a dense representation when it becomes more efficient.

### `P4HyperLogLog`

A P4HyperLogLog sketch is similar to [HyperLogLog](/sql/reference/types#hyperloglog), but it starts (and remains)
