...
The `name` of the routine. [Inline routines](/sql/routines/introduction#inline-routines) can use a simple
string. [Catalog routines](/sql/routines/introduction#catalog-routines) must qualify the name of the catalog
and schema.

{/* TODO: admin stuff? */}
{/* , delimited by `.`, to store the routine or rely on the [default
...
data types `data_type`, see [data type](/sql/reference/types). An empty list, specified as
`()` is also valid.

The `type` value after the `RETURNS` keyword identifies the [data
type](/sql/reference/types) of the routine output.
...
functions. By default, routines are assume to have a deterministic
behavior.
