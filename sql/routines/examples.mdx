---
title: Example SQL routines
---


After learning about [SQL routines from the
introduction](/sql/routines/introduction), the following sections show numerous
examples of valid SQL routines. The routines are suitable as [inline
routines](/sql/routines/introduction#inline-routines) or [catalog routines](/sql/routines/introduction#catalog-routines), after
...
* [function](/sql/routines/function) for general SQL routine declaration
* [begin](/sql/routines/begin) and [declare](/sql/routines/declare) for routine blocks
* [set](/sql/routines/set) for assigning values to variables
* [return](/sql/routines/return) for returning routine results
* [case](/sql/routines/case) and [if](/sql/routines/if) for conditional flows
* [loop](/sql/routines/loop), [repeat](/sql/routines/repeat), and [while](/sql/routines/while) for looping constructs
* [iterate](/sql/routines/iterate) and [leave](/sql/routines/leave) for flow control
...
```
{/* TODO: This links to installation/deployment#config-properties */}
...
Trino includes a built-in function called `human_readable_seconds` that
...
statement with a [`CASE` expression](/sql/functions/conditional#case-expression) and multiple function
...
Trino already has a built-in `bar()` [color function](/sql/functions/color), but
...
Trino already has a built-in [aggregate function](/sql/functions/aggregate) called
